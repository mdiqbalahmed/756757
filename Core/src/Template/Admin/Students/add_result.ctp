...
<body>
    ...
    <?php if (isset($students) && isset($scms_term_course_cycle_parts)) {
        ...
        <?php echo $this->Form->create('', ['type' => 'file']); ?>
        <div class="table-responsive input_result_table mt-10">
            <table class="table table-striped table-hover table-bordered" style="border: 1px solid #dee2e6;">
                        ...
                        <?php foreach ($scms_term_course_cycle_parts as $part) { ?>
                            <th style="min-width: 210px;">
                                ...
                                <?php if ($part['term_course_cycle_part_type_id'] != 9999) { ?>
                                    <input type="text"
                                        id="<?php echo $part['term_course_cycle_part_type_name'] . '_persentage'; ?>">
                                <?php } ?>
                            </th>
                        <?php } ?>
                    </tr>
                ...
                <tbody>
                    <?php foreach ($students as $student) { ?>
                        <tr class="single_row">
                            <input name="student_term_course_cycle_id[<?php echo $student['student_term_course_cycle_id'] ?>]"
                                type="hidden" class="" value='<?php echo $student['student_term_course_cycle_id'] ?>'>
                            ...
                            <?php foreach ($student['mark'] as $part) { ?>
                                <td>
                                    <input
                                        name="<?php echo 'mark' . '[' . $student['student_term_course_cycle_id'] . ']' . '[' . $part['term_course_cycle_part_id'] . '][' . $part['term_course_cycle_part_mark_id'] . ']'; ?>"
                                        type="text" class="<?php echo $part['term_course_cycle_part_type_name']; ?>"
                                        value="<?php echo $part['obtail_marks']; ?>" <?php if ($part['term_course_cycle_part_type_id'] == 9999) { ?> readonly="true" <?php } ?>
        ...
        <?php foreach ($scms_term_course_cycle_parts as $part) { ?>
            <input name="max_marks[<?php echo $part['term_course_cycle_part_type_name'] ?>]" type="hidden"
                class="max_marks-<?php echo $part['term_course_cycle_part_type_name'] ?>" value='<?php echo $part['mark'] ?>'>
        <?php } ?>
        <div class="mt-4 mb-2 text-center">
            <button type="submit" class="btn btn-info px-5"><?= __d('setup', 'Submit') ?></button>
            <?php echo $this->Form->end(); ?>
        </div>
        </div>
    <?php } ?>
    ...
    function getTermAjax() {
     ...
     function single_row(value) {
        ...
        if (marks > max_mark) {
            ...
            value.style.backgroundColor = '#f8d7da'; // red background
            value.value = null;
        } else {
            value.style.backgroundColor = '#e3ffe9ff'; // light green
        }
