                ...
                <div class="schoolIdentity">
                    ...
                    <div class="overlays">
                        <div class="textOverlay" style="border-image: url('<?= $this->Url->image($borderImage) ?>') 18 round;">
                            ...
                            <p class="title_exam_2"> <?php echo $exam_title[2]; ?></p>
                        </div>
                    </div>
                    <!-- end of hdrText -->
                            ...
                            <?php if (isset($student['merge_filter_course'])) { ?>
                                <?php foreach ($student['merge_filter_course'] as $merge_course) { ?>
                                    <tr>
                                        ...
                                        $gpData = $merge_course['1st_course']['table_data']['GP'];
                                        $gradeData = $merge_course['1st_course']['table_data']['Grade'];

                                        // Remove Grade and GP from the original array to avoid duplication
                                        unset($merge_course['1st_course']['table_data']['Grade']);
                                        unset($merge_course['1st_course']['table_data']['GP']);


                                        $inserted = false;
                                        $newTableData = [];
                                        foreach ($merge_course['1st_course']['table_data'] as $key => $table_data) {
                                            if (!$inserted && $key === 'GP') {
                                                // Insert Grade just before GP
                                                $newTableData['Grade'] = $gradeData;
                                                $newTableData['GP'] = $gpData;
                                                $inserted = true;
                                            }
                                            $newTableData[$key] = $table_data;
                                        }

                                        // If GP was not found, append Grade and GP at the end
                                        if (!$inserted) {
                                            $newTableData['GP'] = $gpData;
                                            $newTableData['Grade'] = $gradeData;
                                        }

                                        $merge_course['1st_course']['table_data'] = $newTableData;

                                        // Start rendering the table
                                        foreach ($merge_course['1st_course']['table_data'] as $table_data) { ?>
...
<script>
    document.querySelectorAll('.watermarked').forEach(function(el) {
        el.dataset.watermark = (el.dataset.watermark + ' ').repeat(1200);
    });
</script>